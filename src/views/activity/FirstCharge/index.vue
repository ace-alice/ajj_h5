  ...
  <div class="tiro-task">
    ...
    <div class="tiro-task-box">
      ...
      <div class="tiro-task-table">
        ...
        >
          <div>{{ content.total_recharge }}</div>
          <div>{{ content.amount }}</div>
          <div>{{ activityInfo?.shuffle_rate || '' }} 倍</div>
        </div>
      </div>
...
export default defineComponent({
  ...
  setup() {
    ...
    const friendInvitationImg = require('@/assets/activity/first-charge-header.png');
    // ，首次充值后如有提款操作则无法申请此活动
    const rules = ref([
      '1.每位新会员充值金额≥1000元即可在活动页面申请此首充活动，首次充值后如有提款操作则无法申请此活动。',

      '2.首充成功后请在活动页面领取奖金，会员需完成对应有效流水即可提款，此活动不可与其它活动共享。例：本金1000元，奖金58元 （1000+58）*6=6348元',
    ...
    function getStatus() {
      ...
      request
        .value({
          url: '/index/Activity/memberFirstRechargeStatus',
          method: 'get'
        })
        .then((res: any) => {
          if (+res.code === 1) {
