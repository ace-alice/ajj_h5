<template>
  <div class="v-activities-item">
    ...
    <van-dialog
      v-model:show="vipActivityDialogShow"
      title="提示"
      show-cancel-button
      :style="{ width: '70%' }"
      confirm-button-text="立即领取"
      cancel-button-text="稍后再领"
      @confirm="vipActivityDialogConfirm"
      @cancel="vipActivityDialogShow = false"
    >
      <div class="vip-activity-dialog-main">
        您即将领取{{ dealWithType }}{{ priceToFixed(item.reward_amount) }}元，领取成功
...
import { getToken, parseTime } from '@/utils/index.js'
...
export default defineComponent({
  ...
  setup(props) {
    ...
    const vipActivityDialogShow = ref(false)
    ...
    const getItRightNow = () => {
      vipActivityDialogShow.value = true
    }
    const vipActivityDialogConfirm = () => {
      createRequestFun('2', getToken())({ url: props.item.url, method: 'post' }).then((res: any) => {
        if (+res.code === 1) {
          proxy.mittBus.emit('vip-activity', 1)
    ...
    return {
      parseTime,
      letterDefaultImage,
      vipActivityDialogShow,
      dealWithType,
      priceToFixed,
      getItRightNow,
      vipActivityDialogConfirm
    }
  }
})
</script>
