<template>
  <div class="v-account-password">
    <PubHeader :right-icon="false" :header-title="+userInfo.funds_password === 0 ? '设置交易密码' : '修改交易密码'" />
      ...
      <van-form ref="accountForm" @submit="updateAccountPwd">
        <van-cell-group inset>
          <van-field
            ...
            name="password"
            :label="+userInfo.funds_password === 0 ? '交易密码' : '新密码'"
            placeholder="请输入6-20位数字或英文"
            autocomplete="off"
            maxlength="20"
            :rules="[{validator: transactionPwdValidator}]"
          ...
          <van-field
            ...
            label="验证码"
            placeholder="请输入验证码"
            :rules="[{ validator: transactionCodeValidator }]"
          >
            <template #button>
...
import { updateTransactionPassword, pubCode } from '@/api/setting.js'
...
export default defineComponent({
  ...
  setup() {
    ...
    const transactionPwdValidator = (value: any) => {
      ...
      if (value === '') {
        ...
        return '交易密码不可使用超过3个相同的字母或数字'
      } else if (!reg1.test(value)) {
        return '交易密码必须包含字母和数字'
      } else if (value.indexOf(userInfo.value.phone) !== -1) {
        return '交易密码不可包含用户手机号'
      } else if (!reg2.test(value)) {
        return '交易密码不可使用超过3个连续数字'
      } else {
        return true
      }
    }
     ...
     * 验证码验证
     */
    const transactionCodeValidator = (value: any) => {
      const reg = /^\d{6}$/
      if (value === '') {
        return '验证码不能为空'
      } else if (!reg.test(value)) {
    ...
    const updateAccountPwd = async() => {
      await updateTransactionPassword(state).then((res: any) => {
        if (+res.code === 1) {
          Toast.success({
            message: `交易密码${+userInfo.value.funds_password === 0 ? '设置' : '修改'}成功`,
            duration: 1000
          })
          getPersonalInfo().then((resp: any) => {
            if (+resp.code === 1) {
              store.commit('user/SET_USER_INFO', JSON.stringify(resp.data))
              toPath({ name: 'Setting' })
            }
          }).catch((errp: any) => {
            Toast.fail(errp.msg || errp.message)
          })
        }
      }).catch((err: any) => {
        Toast.fail(err.msg || err.message)
